// Comprehensive Vietnamese diacritics normalization for global search
...
// Match text using Vietnamese-friendly search
...
export function calculateSearchScore(text: string, searchTerm: string): number {
  ...
  // Exact match
  if (normalizedText === normalizedSearch) score += 100;
  
  // Starts with
  if (normalizedText.startsWith(normalizedSearch)) score += 50;
  
  // Contains
  if (normalizedText.includes(normalizedSearch)) score += 25;
  
  // Word boundary match
  const words = normalizedText.split(/\s+/);
  if (words.some(word => word.startsWith(normalizedSearch))) score += 30;
