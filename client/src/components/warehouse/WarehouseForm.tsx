...
export function WarehouseForm({
  ...
  // Fetch next warehouse code when in add mode
  useEffect(() => {
    if (mode === 'add') {
      const fetchNextCode = async () => {
        try {
          const response = await fetch('/api/warehouses/next-code', { credentials: 'include' });
          if (response.ok) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      className={`mt-1 ${mode === 'add' ? 'bg-slate-50 dark:bg-slate-700 font-medium' : ''}`}
                      data-testid="input-warehouse-code"
                    />
                    {mode === 'add' && (
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                        {t('warehouse:codeAutoGenerated', 'Auto-generated. This will be the warehouse ID.')}
                      </p>
                    )}
