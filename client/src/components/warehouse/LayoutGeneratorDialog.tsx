...
export function LayoutGeneratorDialog({ open, onOpenChange, warehouseId, onSuccess }: LayoutGeneratorDialogProps) {
  ...
  const layoutSchemaWithTranslation = z.object({
    name: z.string().min(1, t('warehouse:layoutNameRequired')),
    width: z.number().min(1, t('warehouse:widthMustBeAtLeast')),
    length: z.number().min(1, t('warehouse:lengthMustBeAtLeast')),
    rows: z.number().min(1, t('warehouse:rowsMustBeAtLeast')).max(26, t('warehouse:rowsMaximum')),
    columns: z.number().min(1, t('warehouse:columnsMustBeAtLeast')),
    binWidth: z.number().min(0.1, t('warehouse:binWidthMustBeAtLeast')),
    binHeight: z.number().min(0.1, t('warehouse:binHeightMustBeAtLeast')),
    aisleWidth: z.number().min(0, t('warehouse:aisleWidthCannotBeNegative')),
    binCapacity: z.number().min(1, t('warehouse:binCapacityMustBeAtLeast')),
  });
  
  const form = useForm<LayoutFormData>({
    resolver: zodResolver(layoutSchemaWithTranslation),
    defaultValues: {
      name: t('warehouse:autoGeneratedLayout'),
      width: 10,
      length: 10,
      rows: 10,
      columns: 10,
  ...
  const generateLayoutMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        ...
        description: error.message || t('warehouse:failedToGenerateLayout'),
        variant: "destructive",
      });
    },
  });
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto" data-testid="dialog-layout-generator">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {t('warehouse:generateWarehouseLayout')}
          </DialogTitle>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>{t('warehouse:layoutName')}</FormLabel>
                  <FormControl>
                    <Input {...field} data-testid="input-layout-name" />
                  </FormControl>
                  <FormDescription>
                    {t('warehouse:layoutNameDescription')}
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:warehouseWidth')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:warehouseLength')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:numberOfRows')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        {...field}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:numberOfColumns')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        {...field}
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:binWidth')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:binHeight')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
            ...
            <div className="grid grid-cols-2 gap-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:aisleWidth')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        step="0.1"
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{t('warehouse:binCapacity')}</FormLabel>
                    <FormControl>
                      <Input
                        type="number"
                        {...field}
            ...
            <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              <h4 className="font-semibold text-sm mb-2">{t('warehouse:layoutPreview')}</h4>
              <div className="space-y-1 text-sm text-muted-foreground">
                <p data-testid="text-estimated-bins">{t('warehouse:totalBinsLabel', { count: estimatedBins })}</p>
                <p>{t('warehouse:gridLabel', { rows: form.watch("rows"), columns: form.watch("columns") })}</p>
                <p>{t('warehouse:binCodesLabel', { start: 'A1', end: `${String.fromCharCode(64 + form.watch("rows"))}${form.watch("columns")}` })}</p>
              </div>
            </div>
