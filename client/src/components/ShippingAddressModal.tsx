...
export function ShippingAddressModal({
  ...
  const defaultTitle = title || t('common:addShippingAddress');
  const defaultDescription = description || t('common:enterShippingAddressDetails');
  ...
  const parseAddressMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      ...
      if (data?.fields) {
        ...
        setRawAddress('');
        toast({
          title: t('common:addressParsed'),
          description: t('common:filledWithConfidence', { confidence: data.confidence || 'medium' }),
        });
      }
    },
    onError: (error: any) => {
      ...
      toast({
        ...
        description: error?.message || t('common:couldNotParseAddress'),
      });
    },
  });
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          <div>
            ...
            <Input
              ...
              placeholder={t('common:autoGeneratedFromAddressFieldsEditable')}
              data-testid="input-shipping-label"
            />
            <p className="text-xs text-muted-foreground mt-1">
              {t('common:autoGeneratedFromAddressFieldsEditable')}
            </p>
          </div>

          {/* Smart Paste Section */}
          <div className="space-y-2">
            <Label>{t('common:smartPaste')}</Label>
            <p className="text-sm text-muted-foreground">
              {t('common:pasteAnyAddressInfo')}
            </p>
            <Textarea
              value={rawAddress}
              onChange={(e) => setRawAddress(e.target.value)}
              placeholder={t('common:addressDetailsExample')}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              {form.formState.errors.firstName && (
                <p className="text-xs text-red-500 mt-1">{t('common:firstNameIsRequired')}</p>
              )}
            </div>
            <div>
              ...
              {form.formState.errors.lastName && (
                <p className="text-xs text-red-500 mt-1">{t('common:lastNameIsRequired')}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              <Label htmlFor="streetNumber">{t('common:number')}</Label>
              <Input
                id="streetNumber"
            ...
            <div>
              ...
              <div className="relative">
                <Input
                  ...
                  placeholder={t('common:typeToSearchCountries')}
          ...
          {existingAddresses.length > 0 && (
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="isPrimary">{t('common:setAsPrimaryAddress')}</Label>
            </div>
          )}
