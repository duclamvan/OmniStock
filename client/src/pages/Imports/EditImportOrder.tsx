      ...
      <div className="sticky top-0 z-10 bg-background border-b md:relative md:border-0">
        <div className="flex items-center justify-between p-4 md:p-0">
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div>
              <h1 className="text-lg md:text-2xl font-semibold">{t('editImportOrder')}</h1>
              <p className="text-xs md:text-sm text-muted-foreground">{t('orderNumber')}: #{order.orderNumber}</p>
            </div>
          </div>
          <Badge className="text-xs md:text-sm">{t(`status.${order.status}`)}</Badge>
        </div>
      </div>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-sm">{t('status.label')} *</FormLabel>
                    <Select onValueChange={field.onChange} value={field.value}>
                      <FormControl>
                        <SelectTrigger className="h-10">
                          <SelectValue />
                      ...
                      {item.receivedQuantity !== undefined && (
                        <div className="text-xs">
                          <Badge variant={item.receivedQuantity === item.quantity ? "default" : "secondary"}>
                            {item.receivedQuantity}/{item.quantity} {t('receivedQty')}
                          </Badge>
                        </div>
                      )}
                      {item.weight && (
