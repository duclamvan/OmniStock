...
- **PPL CZ CPL API**: Automated shipping label generation for PPL courier service, using OAuth2 authentication. Supports creating, canceling (both all shipments and individual packages), and retrieving shipping labels with tracking numbers. Includes full dobírka (cash on delivery) support with configurable COD amount and currency (CZK/EUR/USD). Individual shipment cancellation tracks cancelled packages separately, allowing partial order cancellation with automatic UI updates.
