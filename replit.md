...
## Frontend
The frontend is a React and TypeScript application, built with Vite. It uses Shadcn/ui (Radix UI primitives), Tailwind CSS for styling, TanStack Query for state management, Wouter for routing, and React Hook Form with Zod for form validation. The UI/UX emphasizes mobile-first responsiveness, card-based layouts, sticky navigation, and clear visual separation. It features a professional queue-based Pick & Pack workflow system with kanban visualization, modular component architecture (StatusBoard, MobileTaskView, StationWorkspace), and advanced warehouse mapping with a 2D floor plan. The architecture promotes unified form components for both creation and editing and feature-based modular organization, significantly reducing code duplication.
...
- **Professional Pick & Pack Workflow**: Queue-based workflow management system with order locking (15-min auto-expiring locks, 10-min heartbeat, visual countdown timers), kanban visualization (5 status columns: Pending → Picking → Ready to Pack → Packing → Complete), mobile-first picker interface with location guidance, keyboard shortcuts (Space/Enter/Arrows), optimized pick path routing, and barcode scanning, desktop-optimized packer workstation with verification checklist and carton selection, batch picking support via pick waves with multi-order selection dialog, priority management (rush/high/medium/low) with real-time notifications, employee performance tracking with metrics dashboard and leaderboards, real-time collaboration indicators showing active workers, handoff notes between pickers and packers, intelligent notification system for rush orders and expiring locks, real-time queue updates (30s polling), and comprehensive event logging for audit trail. Implemented with modular architecture at client/src/features/pick-pack/ reducing main page from 6,647 lines to 274 lines (96% reduction).
