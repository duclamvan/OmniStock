...
- **Product Bundles**: Comprehensive bundle management system with standalone creation page, robust product variant support, multiple pricing modes (percentage discount, fixed amount, per-item discount, manual pricing), advanced variant selection (search, range selection, custom input), expandable variant display with individual delete capabilities, and simplified card-based UI for managing bundled product offerings.
