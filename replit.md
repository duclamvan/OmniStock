...
- **Financial & Discount Management**: Advanced discount system, customer-specific pricing, multi-currency support (CZK/EUR/USD/VND/CNY) with automatic exchange rate conversion, landing cost engine, expense tracking, and automatic weighted average landed cost calculation upon receiving. EUR is used as the base currency for all conversions. Currency conversions are centralized through `fxConversionService` which provides consistent X→EUR (divide by rate) and EUR→X (multiply by rate) conversions with NaN safety and missing rate fallbacks.
