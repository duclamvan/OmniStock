...
- **Profit Calculation**: Order profit is calculated as `grandTotal - totalCost`. The `totalCost` is computed at order creation by summing `landingCost × quantity` for all items. **CRITICAL**: Cost capture uses only currency-tagged `importCost{Currency}` fields (e.g., `importCostCzk`, `importCostEur`) - never `latestLandingCost` which has unknown currency and causes incorrect cross-currency calculations.
