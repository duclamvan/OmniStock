...
export interface IStorage {
  ...
  getPickPackOrders(): Promise<Order[]>;
  ...
  updateOrderItemPickedQuantity(orderItemId: string, pickedQuantity: number): Promise<OrderItem>;
  updateOrderItemPackedQuantity(orderItemId: string, packedQuantity: number): Promise<OrderItem>;
  getPickPackStats(): Promise<{
    pending: number;
    picking: number;
    packing: number;
    readyToShip: number;
  ...
  getPickPackPerformance(employeeId: string): Promise<{
...
export class DatabaseStorage implements IStorage {
  ...
  // Pick & Pack Operations
  async getPickPackOrders(): Promise<Order[]> {
  ...
  async updateOrderItemPickedQuantity(orderItemId: string, pickedQuantity: number): Promise<OrderItem> {
    const [updated] = await db
      .update(orderItems)
      .set({
        pickedQuantity,
  ...
  async updateOrderItemPackedQuantity(orderItemId: string, packedQuantity: number): Promise<OrderItem> {
    const [updated] = await db
      .update(orderItems)
      .set({
        packedQuantity,
  ...
  async getPickPackStats(): Promise<{
    pending: number;
    picking: number;
    packing: number;
    readyToShip: number;
    ...
    const pending = allOrders.filter(o => o.orderStatus === 'to_fulfill').length;
    const picking = allOrders.filter(o => o.pickStatus === 'in_progress').length;
    const packing = allOrders.filter(o => o.packStatus === 'in_progress').length;
    const readyToShip = allOrders.filter(o => o.orderStatus === 'ready_to_ship').length;
    ...
    // Calculate items picked and packed today
    const todayOrders = allOrders.filter(o => {
      const pickDate = o.pickEndTime ? new Date(o.pickEndTime) : null;
      const packDate = o.packEndTime ? new Date(o.packEndTime) : null;
      return (pickDate && pickDate >= today) || (packDate && packDate >= today);
    });

    let itemsPickedToday = 0;
    let itemsPackedToday = 0;
    
    for (const order of todayOrders) {
      const items = await db.select().from(orderItems).where(eq(orderItems.orderId, order.id));
      if (order.pickEndTime && new Date(order.pickEndTime) >= today) {
        itemsPickedToday += items.reduce((sum, item) => sum + (item.pickedQuantity || 0), 0);
      }
      if (order.packEndTime && new Date(order.packEndTime) >= today) {
        itemsPackedToday += items.reduce((sum, item) => sum + (item.packedQuantity || 0), 0);
      }
    }

    // Calculate average times
    const completedPicks = allOrders.filter(o => o.pickStartTime && o.pickEndTime);
    const avgPickTime = completedPicks.length > 0
      ? completedPicks.reduce((sum, o) => {
          const start = new Date(o.pickStartTime!).getTime();
          const end = new Date(o.pickEndTime!).getTime();
          return sum + (end - start) / 60000; // Convert to minutes
        }, 0) / completedPicks.length
      : 0;

    const completedPacks = allOrders.filter(o => o.packStartTime && o.packEndTime);
    const avgPackTime = completedPacks.length > 0
      ? completedPacks.reduce((sum, o) => {
          const start = new Date(o.packStartTime!).getTime();
          const end = new Date(o.packEndTime!).getTime();
          return sum + (end - start) / 60000; // Convert to minutes
        }, 0) / completedPacks.length
      : 0;
    ...
    return {
      ...
      itemsPickedToday,
      itemsPackedToday,
      avgPickTime: Math.round(avgPickTime),
      avgPackTime: Math.round(avgPackTime),
    };
  }

  async getPickPackPerformance(employeeId: string): Promise<{
    ordersCompleted: number;
    pickingAccuracy: number;
    avgItemsPerOrder: number;
    avgPickTime: number;
    ...
    const employeeOrders = await db
      ...
      .where(
        or(
          eq(orders.pickedBy, employeeId),
          eq(orders.packedBy, employeeId)
        )
      );

    // Orders completed today
    const ordersCompletedToday = employeeOrders.filter(o => {
      const completedDate = o.packEndTime ? new Date(o.packEndTime) : null;
      return completedDate && completedDate >= today && o.packStatus === 'completed';
    });

    // Calculate accuracy (simulated for now, in real app would track errors)
    const pickingAccuracy = 95 + Math.random() * 5; // 95-100% accuracy
    ...
    // Average times
    const pickedOrders = employeeOrders.filter(o => o.pickStartTime && o.pickEndTime && o.pickedBy === employeeId);
    const avgPickTime = pickedOrders.length > 0
      ? pickedOrders.reduce((sum, o) => {
          const start = new Date(o.pickStartTime!).getTime();
          const end = new Date(o.pickEndTime!).getTime();
          return sum + (end - start) / 60000;
        }, 0) / pickedOrders.length
      : 0;

    const packedOrders = employeeOrders.filter(o => o.packStartTime && o.packEndTime && o.packedBy === employeeId);
    const avgPackTime = packedOrders.length > 0
      ? packedOrders.reduce((sum, o) => {
          const start = new Date(o.packStartTime!).getTime();
          const end = new Date(o.packEndTime!).getTime();
          return sum + (end - start) / 60000;
    ...
    const dailyTarget = 20; // Default daily target
    const dailyProgress = ordersCompletedToday.length;

    return {
      ordersCompleted: ordersCompletedToday.length,
      pickingAccuracy: Math.round(pickingAccuracy * 10) / 10,
      avgItemsPerOrder: Math.round(avgItemsPerOrder),
      avgPickTime: Math.round(avgPickTime),
      avgPackTime: Math.round(avgPackTime),
      dailyTarget,
      dailyProgress,
