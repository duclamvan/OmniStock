...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      const ordersWithItems = await Promise.all(
        orders.map(async (order) => {
          ...
          const pickedItems = itemsWithProducts.reduce((sum, item) => sum + (item.pickedQuantity || 0), 0);
          const packedItems = itemsWithProducts.reduce((sum, item) => sum + (item.packedQuantity || 0), 0);
          ...
          return {
            ...
            pickedItems,
            packedItems,
            priority: order.shippingMethod?.includes('Express') ? 'high' : 
                     order.shippingMethod?.includes('Standard') ? 'medium' : 'low',
          };
        })
      );
  ...
  app.get('/api/orders/pick-pack/stats', async (req, res) => {
    try {
      const stats = await storage.getPickPackStats();
      res.json(stats);
    } catch (error) {
      console.error('Error fetching pick-pack stats:', error);
      res.status(500).json({ message: "Failed to fetch pick-pack statistics" });
    }
  });

  app.get('/api/orders/pick-pack/performance/:employeeId', async (req, res) => {
    try {
      const { employeeId } = req.params;
      const performance = await storage.getPickPackPerformance(employeeId);
  ...
  app.post('/api/orders/:id/start-picking', async (req, res) => {
    try {
      const { id } = req.params;
      const { employeeId } = req.body;
      
      const updatedOrder = await storage.startPickingOrder(id, employeeId || 'Employee #001');
      
      await storage.createUserActivity({
        id: nanoid(),
        userId: employeeId || 'Employee #001',
        type: 'order',
        action: 'Started picking order',
      ...
      res.status(500).json({ message: "Failed to start picking" });
    }
  });

  app.post('/api/orders/:id/complete-picking', async (req, res) => {
    try {
      const { id } = req.params;
      const updatedOrder = await storage.completePickingOrder(id);
      
      await storage.createUserActivity({
        id: nanoid(),
        userId: updatedOrder.pickedBy || 'Employee #001',
        type: 'order',
        action: 'Completed picking order',
        description: `Completed picking order ${updatedOrder.orderId}`,
        entityId: id,
        entityType: 'order',
      });
      ...
      res.status(500).json({ message: "Failed to complete picking" });
    }
  });

  app.post('/api/orders/:id/start-packing', async (req, res) => {
    try {
      const { id } = req.params;
      const { employeeId } = req.body;
      
      const updatedOrder = await storage.startPackingOrder(id, employeeId || 'Employee #001');
      
      await storage.createUserActivity({
        id: nanoid(),
        userId: employeeId || 'Employee #001',
        type: 'order',
        action: 'Started packing order',
        description: `Started packing order ${updatedOrder.orderId}`,
        entityId: id,
        entityType: 'order',
      });
      ...
      res.status(500).json({ message: "Failed to start packing" });
    }
  });

  app.post('/api/orders/:id/complete-packing', async (req, res) => {
    try {
      const { id } = req.params;
      const updatedOrder = await storage.completePackingOrder(id);
      
      await storage.createUserActivity({
        id: nanoid(),
        userId: updatedOrder.packedBy || 'Employee #001',
        type: 'order',
        action: 'Completed packing order',
        description: `Completed packing order ${updatedOrder.orderId}`,
        entityId: id,
        entityType: 'order',
      });
  ...
  app.patch('/api/order-items/:id/picked', async (req, res) => {
    try {
      ...
      const updatedItem = await storage.updateOrderItemPickedQuantity(id, pickedQuantity);
  ...
  app.patch('/api/order-items/:id/packed', async (req, res) => {
    try {
      ...
      const updatedItem = await storage.updateOrderItemPackedQuantity(id, packedQuantity);
      res.json(updatedItem);
    } catch (error) {
