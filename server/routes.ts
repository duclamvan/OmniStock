      ...
      for (let i = 0; i < data.length; i++) {
        ...
        try {
          ...
          const customerData = {
            ...
            type: row['Customer Type'] || row['Type'] || 'regular',
            // Support both new (Address) and legacy (Street) column names
            address: row['Address'] || row['Street'] || null,
            city: row['City'] || null,
            // Support both new (Zip Code) and legacy (Postal Code) column names
            zipCode: row['Zip Code'] || row['Postal Code'] || null,
            country: row['Country'] || null,
            // Facebook fields - support multiple naming conventions
            facebookId: row['Facebook ID'] || row['FacebookID'] || row['facebook_id'] || null,
            facebookName: row['Facebook Name'] || row['FacebookName'] || row['facebook_name'] || null,
            facebookUrl: row['Facebook URL'] || row['FacebookURL'] || row['facebook_url'] || null,
            ico: row['ICO'] || null,
            dic: row['DIC'] || null,
            vatId: row['VAT ID'] || row['VatID'] || null,
            vatNumber: row['VAT Number'] || null,
            taxId: row['Tax ID'] || null,
            preferredCurrency: row['Preferred Currency'] || row['Currency'] || null,
            preferredLanguage: row['Preferred Language'] || row['Language'] || null,
            notes: row['Notes'] || null,
            // Billing fields
            billingFirstName: row['Billing First Name'] || null,
            billingLastName: row['Billing Last Name'] || null,
            billingCompany: row['Billing Company'] || row['Company'] || null,
            billingEmail: row['Billing Email'] || null,
            billingTel: row['Billing Phone'] || row['Billing Tel'] || null,
            billingStreet: row['Billing Street'] || null,
            billingStreetNumber: row['Billing Street Number'] || null,
            billingCity: row['Billing City'] || null,
            billingZipCode: row['Billing Zip Code'] || null,
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        try {
          ...
          const customerData: any = {
            ...
            billingEmail: item.billingEmail || null,
            billingTel: item.billingTel || null,
            billingStreet: item.billingStreet || null,
            billingStreetNumber: item.billingStreetNumber || null,
...
- Return ONLY the JSON, no additional text`;
  ...
  const { createPPLShipment, createPPLSingleShipment, getPPLLabelByShipmentId, getPPLBatchStatus, getPPLLabel, searchPPLAccessPoints } = await import('./services/pplService');
  ...
  // Refresh PPL tracking numbers - fetches real tracking numbers from PPL API
  app.post('/api/orders/:orderId/ppl/refresh-tracking', isAuthenticated, async (req, res) => {
    try {
      ...
      const pplLabels = labels.filter((l: any) => 
        l.carrier === 'PPL' && 
        l.status === 'active' &&
        l.trackingNumbers?.some((tn: string) => tn.startsWith('PENDING-'))
      );
      
      if (pplLabels.length === 0) {
        return res.json({ 
          success: true, 
          message: 'No pending PPL tracking numbers to refresh',
          updated: 0
        });
      }
      ...
      for (const label of pplLabels) {
        ...
        if (shipmentNumbers.length > 0) {
          // Update the shipment label
          await storage.updateShipmentLabel(label.id, {
            trackingNumbers: shipmentNumbers
          });
          
          // Update cartons with real tracking numbers
          const cartons = await storage.getOrderCartons(orderId);
          for (let i = 0; i < cartons.length; i++) {
            const carton = cartons[i];
            if (carton.trackingNumber?.startsWith('PENDING-')) {
              const realTrackingNumber = shipmentNumbers[i] || shipmentNumbers[0];
              await storage.updateOrderCarton(carton.id, {
          ...
          // Update order's pplShipmentNumbers
          if (order.pplShipmentNumbers?.some((tn: string) => tn.startsWith('PENDING-'))) {
            await storage.updateOrder(orderId, {
              pplShipmentNumbers: shipmentNumbers as any
            });
          }
          
          // Update or create shipment tracking records
          const trackingRecords = await db.query.shipmentTracking.findMany({
            where: eq(shipmentTracking.orderId, orderId)
          });
          
          const cartonsAfterUpdate = await storage.getOrderCartons(orderId);
          
          // Update existing records with PENDING tracking numbers
          for (const record of trackingRecords) {
            if (record.trackingNumber?.startsWith('PENDING-')) {
              const newTrackingNumber = shipmentNumbers.find((_, idx) => 
                record.trackingNumber === `PENDING-${batchId?.slice(0, 8)}-${idx + 1}`
              ) || shipmentNumbers[0];
              ...
              if (newTrackingNumber) {
                await db.update(shipmentTracking)
                  .set({ 
                    ...
                    statusCode: 'created',
                    statusLabel: 'Tracking number received',
                    lastSyncAt: new Date()
                  })
                  .where(eq(shipmentTracking.id, record.id));
              }
            }
          }
          
          // Create new tracking records for cartons that don't have one yet
          // Track by cartonId to avoid duplicates after updating existing records
          const existingCartonIds = new Set(trackingRecords.map(r => r.cartonId).filter(Boolean));
          for (const carton of cartonsAfterUpdate) {
            if (carton.trackingNumber && 
                !carton.trackingNumber.startsWith('PENDING-') && 
                !existingCartonIds.has(carton.id)) {
          ...
          results.push({ 
            labelId: label.id, 
            pending: true, 
            message: 'Tracking numbers not yet available from PPL' 
          });
        }
      }
      ...
      res.json({
        success: true,
        updated: updatedCount,
        total: pplLabels.length,
        results
      });
    } catch (error) {
      console.error('Error refreshing PPL tracking:', error);
      res.status(500).json({ 
        error: error instanceof Error ? error.message : 'Failed to refresh PPL tracking' 
      });
    }
  });
  ...
  app.post('/api/shipping/create-label-for-carton', isAuthenticated, async (req, res) => {
    try {
      ...
      const { createPPLShipment, getPPLLabel } = await import('./services/pplService');
    ...
    try {
      ...
      if (useSingleEndpoint) {
        ...
        try {
          ...
          return res.status(500).json({ 
            error: singleError.message || 'Failed to create PPL shipment',
      ...
      // Update order with PPL data
      await storage.updateOrder(orderId, {
        pplBatchId: batchId || shipmentId, // Use shipmentId for single shipment, batchId for batch
        pplShipmentNumbers: shipmentNumbers as any,
        pplLabelData: {
      ...
      const savedLabel = await storage.createShipmentLabel({
        ...
        carrier: 'PPL',
        trackingNumbers: shipmentNumbers,
        batchId: batchId || shipmentId, // Use shipmentId if no batchId
        labelBase64: label.labelContent,
        labelData: {
          shipments,
          referenceId: order.orderId,
          ...
          cartonIds,
          hasCOD,
          shipmentId: shipmentId || undefined // Store shipmentId for single shipments
        },
        shipmentCount: shipmentNumbers.length,
        status: 'active'
      });
      ...
      for (let i = 0; i < cartons.length; i++) {
        const carton = cartons[i];
        const trackingNumber = shipmentNumbers[i] || shipmentNumbers[0];
        await storage.updateOrderCarton(carton.id, { trackingNumber });
      }

      // Create shipmentTracking records for immediate tracking
      for (let i = 0; i < cartons.length; i++) {
        const carton = cartons[i];
        const trackingNumber = shipmentNumbers[i] || shipmentNumbers[0];
        if (trackingNumber && !trackingNumber.startsWith('PENDING-')) {
          await db.insert(shipmentTracking).values({
            orderId,
            cartonId: carton.id,
            carrier: 'ppl',
  ...
  app.post('/api/orders/:orderId/ppl/retry-label', isAuthenticated, async (req, res) => {
    try {
      const { orderId } = req.params;
      const { getPPLLabel, getPPLBatchStatus, getPPLLabelByShipmentId } = await import('./services/pplService');
      ...
      // Check if we have a batchId or shipmentId to retry with
      const pplLabelData = order.pplLabelData as any;
      const shipmentId = pplLabelData?.shipmentId; // Single shipment ID (from POST /shipment)
      const batchId = !shipmentId ? order.pplBatchId : null;
      
      if (!batchId && !shipmentId) {
        return res.status(400).json({ 
          error: 'No PPL batch ID or shipment ID found. Please create a new label instead.' 
        });
      }

      // Check if label already exists in labelData (avoid duplicates)
      if (pplLabelData?.labelBase64) {
        return res.status(400).json({ 
      ...
      // Also check shipment_labels table for existing labels
      const existingLabels = await storage.getShipmentLabelsByOrderId(orderId);
      const existingLabelForId = existingLabels.find(l => l.batchId === (batchId || shipmentId));
      if (existingLabelForId) {
        return res.status(400).json({ 
          error: 'Label already exists. Refreshing order data.',
          batchId: batchId || shipmentId
        });
      }
  ...
  app.post('/api/system/factory-reset', requireRole(['administrator']), async (req: any, res) => {
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        // Use .returning() to get deleted rows array and count length
        
        // Level 1: Leaf tables (no children)
        deletionSummary.ticketComments = (await tx.delete(ticketComments).returning()).length;
        deletionSummary.orderFulfillmentLogs = (await tx.delete(orderFulfillmentLogs).returning()).length;
        deletionSummary.customerBadges = (await tx.delete(customerBadges).returning()).length;
        deletionSummary.supplierFiles = (await tx.delete(supplierFiles).returning()).length;
        deletionSummary.warehouseFiles = (await tx.delete(warehouseFiles).returning()).length;
        deletionSummary.warehouseFinancialContracts = (await tx.delete(warehouseFinancialContracts).returning()).length;
        deletionSummary.productFiles = (await tx.delete(productFiles).returning()).length;
        deletionSummary.orderFiles = (await tx.delete(orderFiles).returning()).length;
        deletionSummary.aiLocationSuggestions = (await tx.delete(aiLocationSuggestions).returning()).length;
        deletionSummary.stockAdjustmentRequests = (await tx.delete(stockAdjustmentRequests).returning()).length;
        ...
        deletionSummary.bundleItems = (await tx.delete(bundleItems).returning()).length;
        deletionSummary.productTieredPricing = (await tx.delete(productTieredPricing).returning()).length;
        deletionSummary.productVariants = (await tx.delete(productVariants).returning()).length;
        deletionSummary.orderItems = (await tx.delete(orderItems).returning()).length;
        deletionSummary.serviceItems = (await tx.delete(serviceItems).returning()).length;
        deletionSummary.preOrderItems = (await tx.delete(preOrderItems).returning()).length;
        deletionSummary.packingMaterialUsage = (await tx.delete(packingMaterialUsage).returning()).length;
        deletionSummary.shipmentLabels = (await tx.delete(shipmentLabels).returning()).length;
        deletionSummary.shipmentItems = (await tx.delete(shipmentItems).returning()).length;
        deletionSummary.shipmentCartons = (await tx.delete(shipmentCartons).returning()).length;
        deletionSummary.consolidationItems = (await tx.delete(consolidationItems).returning()).length;
        deletionSummary.costAllocations = (await tx.delete(costAllocations).returning()).length;
        deletionSummary.receiptItems = (await tx.delete(receiptItems).returning()).length;
        ...
        deletionSummary.orderCartons = (await tx.delete(orderCartons).returning()).length;
        deletionSummary.productBundles = (await tx.delete(productBundles).returning()).length;
        deletionSummary.tickets = (await tx.delete(tickets).returning()).length;
        deletionSummary.landedCosts = (await tx.delete(landedCosts).returning()).length;
        deletionSummary.receipts = (await tx.delete(receipts).returning()).length;
        deletionSummary.shipmentCosts = (await tx.delete(shipmentCosts).returning()).length;
        deletionSummary.productCostHistory = (await tx.delete(productCostHistory).returning()).length;
        ...
        deletionSummary.customerShippingAddresses = (await tx.delete(customerShippingAddresses).returning()).length;
        deletionSummary.customerBillingAddresses = (await tx.delete(customerBillingAddresses).returning()).length;
        deletionSummary.products = (await tx.delete(products).returning()).length;
        deletionSummary.customers = (await tx.delete(customers).returning()).length;
        deletionSummary.shipments = (await tx.delete(shipments).returning()).length;
        deletionSummary.consolidations = (await tx.delete(consolidations).returning()).length;
        deletionSummary.importPurchases = (await tx.delete(importPurchases).returning()).length;
        deletionSummary.services = (await tx.delete(services).returning()).length;
        deletionSummary.preOrders = (await tx.delete(preOrders).returning()).length;
        deletionSummary.expenses = (await tx.delete(expenses).returning()).length;
        deletionSummary.discounts = (await tx.delete(discounts).returning()).length;
        deletionSummary.employees = (await tx.delete(employees).returning()).length;
        ...
        if (!keepSettings) {
          ...
          deletionSummary.warehouseTasks = (await tx.delete(warehouseTasks).returning()).length;
          deletionSummary.warehouseLabels = (await tx.delete(warehouseLabels).returning()).length;
          deletionSummary.warehouseLayouts = (await tx.delete(warehouseLayouts).returning()).length;
          // Level 2: Parent infrastructure tables
          deletionSummary.warehouses = (await tx.delete(warehouses).returning()).length;
          deletionSummary.suppliers = (await tx.delete(suppliers).returning()).length;
        ...
        await tx.insert(userActivities).values({
          ...
          description: `Factory reset completed by ${req.user?.firstName || 'Administrator'}. Mode: ${keepSettings ? 'Keep Settings' : 'Full Reset'}. ${totalDeleted} records deleted. Backup: ${backupResult.fileName} (ID: ${backupResult.backupId}, recoverable for 7 days).`,
        });
      }, {
