 ...
 * 
 * Priority order:
 * 1. Exchange rates from shipment's stored exchangeRates (set at calculation time)
 * 2. Cost allocations table (for custom items with calculated allocations)
 * 3. purchaseItems.landingCostUnitBase (for purchase items)
 * 4. Fall back to unit price * 1.15 (15% markup estimate)
 * 
 * Currency handling:
 ...
 * - Exchange rates are fetched from shipment's stored exchangeRates (Frankfurter API rates at calculation time)
 * - Falls back to reasonable defaults if no stored rates available
 */
async function getLandedCostForItem(
  ...
  if (shipmentId) {
    try {
      ...
      if (shipment?.exchangeRates) {
        // exchangeRates is stored as JSON with format: { USD: 1.08, CZK: 25.2, ... }
        // These are EUR→currency rates from Frankfurter API at time of calculation
  ...
  // Priority 2: Check cost allocations table for this item
  ...
  // Priority 4: Fall back to unit price * 1.15 (15% markup estimate)
  // Note: This path is used when no cost allocations or landingCostUnitBase exist
  // unitPrice comes from purchaseItems/customItems which should be in EUR (base currency)
  // If unitPrice is stored in a different currency, this estimate may be inaccurate
  // In production, most items should have proper landingCostUnitBase from LandingCostService
  if (source === 'fallback') {
    landingCostPerUnit = unitPrice * 1.15;
    console.warn('Landed cost fallback used for item - unitPrice assumed to be EUR:', {
      itemType,
      itemId,
      unitPrice,
      estimatedLandedCost: landingCostPerUnit
    });
  }
  ...
  try {
    ...
    for (const item of allReceiptItems) {
      ...
      if (variantAllocations && variantAllocations.length > 0) {
        ...
        // Exchange rates are stored as EUR->currency multipliers (e.g., USD: 1.08 means 1 EUR = 1.08 USD)
        // Extended defaults for all supported currencies
        let exchangeRates: Record<string, number> = { 
        ...
        for (const va of variantAllocations) {
          ...
          // Determine variant unit price: use variant's price if available, otherwise convert parent's price
          let variantUnitPriceOriginal: number;
          if (variantUnitPriceRaw > 0) {
            // Variant has its own price - use it directly
            variantUnitPriceOriginal = variantUnitPriceRaw;
          } else if (variantCurrency === itemPaymentCurrency) {
...
router.post("/purchases", async (req, res) => {
  try {
    ...
    if (items.length > 0) {
      ...
      // OPTIMIZED: Bulk operations instead of per-variant queries
      // ================================================================
      
      // Collect all variant allocations from all items
        ...
        for (const va of allocations) {
          ...
          if (variantSku || variantName) {
            allAllocations.push({
              ...
              sku: variantSku,
              name: variantName,
              unitPrice: va.unitPrice ? parseFloat(va.unitPrice) : null
            });
          }
        }
      }
      ...
      if (allAllocations.length > 0) {
        console.log(`[Purchase Create] Processing ${allAllocations.length} variants in bulk`);
        
        // Get unique product IDs
        const productIds = Array.from(new Set(allAllocations.map(a => a.productId)));
        
        // Fetch ALL existing variants for these products in ONE query
        const existingVariants = await db
          .select({ id: productVariants.id, productId: productVariants.productId, sku: productVariants.sku, name: productVariants.name })
          .from(productVariants)
          .where(inArray(productVariants.productId, productIds));
        
        // Create lookup maps for fast matching
        const variantBySkuKey = new Map<string, { id: string; sku: string | null }>();
        const variantByNameKey = new Map<string, { id: string; name: string | null }>();
        
        for (const v of existingVariants) {
          if (v.sku) {
            variantBySkuKey.set(`${v.productId}:${v.sku}`, { id: v.id, sku: v.sku });
          }
          if (v.name) {
            variantByNameKey.set(`${v.productId}:${v.name}`, { id: v.id, name: v.name });
          }
        }
        
        // Separate allocations into new variants and updates
        const variantsToCreate: Array<any> = [];
        const variantsToUpdate: Array<{ id: string; sku: string }> = [];
        const seenKeys = new Set<string>();
        
        for (const alloc of allAllocations) {
          // Check if already exists by SKU
          const skuKey = alloc.sku ? `${alloc.productId}:${alloc.sku}` : null;
          const nameKey = alloc.name ? `${alloc.productId}:${alloc.name}` : null;
          
          // Deduplicate within this batch
          const uniqueKey = skuKey || nameKey || '';
          if (seenKeys.has(uniqueKey)) continue;
          seenKeys.add(uniqueKey);
          
          const existingBySku = skuKey ? variantBySkuKey.get(skuKey) : null;
          const existingByName = nameKey ? variantByNameKey.get(nameKey) : null;
          
          if (existingBySku) {
            // Already exists, no action needed
          } else if (existingByName) {
            // Found by name, update SKU if we have one
            if (alloc.sku && !existingByName.name) {
              variantsToUpdate.push({ id: existingByName.id, sku: alloc.sku });
            }
          } else {
            // New variant, add to bulk insert list
            variantsToCreate.push({
              productId: alloc.productId,
              name: alloc.name || alloc.sku,
              sku: alloc.sku,
              quantity: 0,
              importCostUsd: alloc.unitPrice ? (alloc.unitPrice * (1 / (req.body.exchangeRates?.USD || 1))).toString() : null,
              importCostEur: alloc.unitPrice ? (alloc.unitPrice * (1 / (req.body.exchangeRates?.EUR || 1))).toString() : null,
              importCostCzk: alloc.unitPrice ? (alloc.unitPrice * (1 / (req.body.exchangeRates?.CZK || 1))).toString() : null,
              createdAt: new Date(),
              updatedAt: new Date()
            });
            
            // Add to lookup to prevent duplicates within this batch
            if (skuKey) variantBySkuKey.set(skuKey, { id: 'pending', sku: alloc.sku });
            if (nameKey) variantByNameKey.set(nameKey, { id: 'pending', name: alloc.name });
          }
        }
        
        // Bulk insert new variants (one query for all)
        if (variantsToCreate.length > 0) {
          await db.insert(productVariants).values(variantsToCreate);
          console.log(`[Purchase Create] Bulk created ${variantsToCreate.length} new variants`);
        }
        
        // Bulk update existing variants that need SKU updates
        if (variantsToUpdate.length > 0) {
          for (const update of variantsToUpdate) {
            await db.update(productVariants)
              .set({ sku: update.sku, updatedAt: new Date() })
              .where(eq(productVariants.id, update.id));
          }
          console.log(`[Purchase Create] Updated ${variantsToUpdate.length} existing variants with SKUs`);
        }
      }
...
router.get("/shipments/storage", async (req, res) => {
  try {
    ...
    // Load items for each shipment (handles both consolidated and direct PO shipments)
    if (formattedShipments.length > 0) {
      const shipmentIds = formattedShipments.map(s => s.id);
      const consolidationIds = formattedShipments.map(s => s.consolidationId).filter(Boolean) as number[];
      
      // Always fetch receipts and receipt items for ALL shipments
      ...
      // Get consolidation items for consolidated shipments
      let itemsByConsolidationId: Record<number, any[]> = {};
      let customItemsMap = new Map<string, any>();
      let purchaseItemsMap = new Map<string, any>();
      ...
      if (consolidationIds.length > 0) {
        ...
        customItemsMap = new Map(customItemsData.map(item => [item.id, item]));
        purchaseItemsMap = new Map(purchaseItemsData.map(item => [item.id, item]));
      ...
      // For non-consolidated shipments, fetch purchase items directly from receipt items
      const directShipmentIds = formattedShipments
        .filter(s => !s.consolidationId)
        .map(s => s.id);
      
      if (directShipmentIds.length > 0) {
        // Collect all purchase item IDs from receipt items of direct shipments
        const directPurchaseItemIds: string[] = [];
        const directCustomItemIds: string[] = [];
        
        for (const shipmentId of directShipmentIds) {
          const receipt = receiptByShipmentId[shipmentId];
          if (receipt) {
            const items = receiptItemsByReceiptId[receipt.id] || [];
            for (const ri of items) {
              if (ri.itemType === 'purchase' && ri.itemId) {
                directPurchaseItemIds.push(ri.itemId);
              } else if (ri.itemType === 'custom' && ri.itemId) {
                directCustomItemIds.push(ri.itemId);
              }
            }
          }
        }
        
        // Fetch missing purchase/custom items not already in maps
        const missingPurchaseIds = directPurchaseItemIds.filter(id => !purchaseItemsMap.has(id));
        const missingCustomIds = directCustomItemIds.filter(id => !customItemsMap.has(id));
        
        const [additionalPurchaseItems, additionalCustomItems] = await Promise.all([
          missingPurchaseIds.length > 0
            ? db.select().from(purchaseItems).where(inArray(purchaseItems.id, missingPurchaseIds))
            : Promise.resolve([]),
          missingCustomIds.length > 0
            ? db.select().from(customItems).where(inArray(customItems.id, missingCustomIds))
            : Promise.resolve([])
        ]);
        
        additionalPurchaseItems.forEach(item => purchaseItemsMap.set(item.id, item));
        additionalCustomItems.forEach(item => customItemsMap.set(item.id, item));
      }
      ...
      formattedShipments = formattedShipments.map(shipment => {
        ...
        if (shipment.consolidationId) {
          // Consolidated shipment: get items from consolidation
          items = itemsByConsolidationId[shipment.consolidationId] || [];
          
          // Merge receipt item data with consolidation item data
          items = items.map(item => {
            const matchingReceiptItem = shipmentReceiptItems.find((ri: any) => {
              if (item.consolidationItemId && ri.itemId === item.consolidationItemId) return true;
              if (ri.sku && item.sku && ri.sku === item.sku) return true;
              if (ri.itemId === item.id) return true;
              return false;
            });
          ...
          // Direct PO shipment: build items from receipt items
          items = shipmentReceiptItems.map((ri: any) => {
            const itemData = ri.itemType === 'purchase' 
              ? purchaseItemsMap.get(ri.itemId)
              : customItemsMap.get(ri.itemId);
            ...
            return {
              ...
              receiptItemId: ri.id,
              receivedQuantity: ri.receivedQuantity ?? 0,
              assignedQuantity: ri.assignedQuantity ?? 0,
              expectedQuantity: ri.expectedQuantity ?? itemData?.quantity ?? 0,
              warehouseLocation: ri.warehouseLocation,
              sku: ri.sku || itemData?.sku,
              name: itemData?.name || ri.sku || 'Unknown Item',
              quantity: ri.expectedQuantity ?? itemData?.quantity ?? 0
            };
          });
        }
  ...
  try {
      ...
      if (shipment.notes && (shipment.notes.includes('Auto-created from Purchase Order PO #') || shipment.notes.includes('Direct shipment from Purchase Order'))) {
        ...
        if (match) {
          ...
          if (purchases.length > 0) {
            ...
            // Convert total paid to EUR using fxConversionService
            const totalPaidEur = fxConversionService.convertToEur(totalPaid, paymentCurrency, exchangeRates);
            ...
            for (const item of purchaseItemsList) {
              ...
              if (variantAllocationsArray.length > 0) {
                ...
                const itemUnitPriceEur = fxConversionService.safeConvertToEur(item.unitPrice, purchaseCurrency, exchangeRates);
            ...
            for (const item of purchaseItemsList) {
              ...
              if (totalPaidEur > 0 && totalUnits > 0) {
                ...
                if (variantAllocationsArray.length > 0) {
                  ...
                  if (unitPriceEur === 0) {
                    unitPriceEur = fxConversionService.safeConvertToEur(item.unitPrice, itemPaymentCurrency, exchangeRates);
                  }
                } else {
                  unitPriceEur = fxConversionService.safeConvertToEur(item.unitPrice, itemPaymentCurrency, exchangeRates);
                }
    ...
    for (const row of itemsWithCartons) {
      ...
      if (sourcePO && sourcePO.totalPaid > 0 && sourcePO.totalQuantity > 0) {
        ...
        const totalPaidEur = fxConversionService.convertToEur(sourcePO.totalPaid, sourcePO.paymentCurrency, exchangeRates);
...
async function getItemAllocationBreakdownWithMethod(
  ...
  try {
    ...
    for (const itemAllocation of itemAllocations) {
      ...
      const unitPriceEur = fxConversionService.safeConvertToEur(itemAllocation.unitPrice, itemPaymentCurrency, exchangeRates);
